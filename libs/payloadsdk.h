...
// 환경변수에서 IP 값을 읽는 함수
inline const char *get_target_ip()
{
    const char *env_ip = std::getenv("GREMSY_UDP_IP_TARGET"); // 환경변수 UDP_IP_TARGET 읽기
    return env_ip ? env_ip : "192.168.168.240";               // 환경변수가 없으면 기본값 반환
}
static char *payload_uart_port = (char *)"/dev/ttyUSB0";
static int payload_uart_baud = 115200;
static const char *udp_ip_target = get_target_ip(); // This is an ip address of the payload
